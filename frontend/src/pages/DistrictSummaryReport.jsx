...
import { toISODate } from "../config/config.js";
...
const DistrictSummaryReport = () => {
  ...
  } = useFetchDistrictReportQuery({ UnitType: unit, toggle: toggleValue });
  const {
  ...
  //   console.log(availableSchemes);

  //   // Filter only the schemes that are available
  //   const validSchemes = availableSchemes.filter(scheme =>
  //     selectedSchemes.includes(scheme)
  //   );

  //   console.log(validSchemes,"check");
  ...
  //   useEffect(() => {
  //   if (!mappingData || Object.keys(mappingData).length === 0) return;

  //   // Only schemes available in district API
  //   const schemesAvailable = Object.keys(mappingData);

  //   // 1. Sync selectedSchemes to only those available from the API
  //   const filteredSchemes = selectedSchemes.filter(s => schemesAvailable.includes(s));
  //   setSelectedSchemes(filteredSchemes.length > 0 ? filteredSchemes : schemesAvailable);

  //   // 2. For currently selected schemes, gather the available KPIs in the district
  //   const kpiOptions = dedupe(
  //     (filteredSchemes.length > 0 ? filteredSchemes : schemesAvailable).flatMap(
  //       scheme => (mappingData[scheme] || []).map(kpiObj => kpiObj["KPI Name"])
  //     )
  //   );

  //   // 3. Sync selectedKPIs to only those available
  //   const filteredKPIs = selectedKPIs.filter(kpi => kpiOptions.includes(kpi));
  //   setSelectedKPIs(filteredKPIs.length > 0 ? filteredKPIs : kpiOptions);
  ...
  useEffect(() => {
    if (
      ...
      setFilteredArray([]);
    } else if (!outputdata || Object.keys(outputdata).length === 0) {
      ...
      if (selectedKPIs.length > 0) {
        Object.keys(filtered).forEach((scheme) => {
          filtered[scheme] = Object.keys(filtered[scheme])
            ...
            .reduce(
              (obj, key) => ({ ...obj, [key]: filtered[scheme][key] }),
              {}
            );
        });
      }
  ...
  const onUpdateUnit = (unit) => {
    ...
    localStorage.removeItem("districtReport");
    dispatch(api.util.invalidateTags(["DistrictReport"]));
  };

  // Table render
  let globalIndex = 0;

  function getLatestDateIfNA(
    outputData,
    schemeKey,
    kpiKey,
    stateName,
    ...
    const allDistrictData = outputData[schemeKey][kpiKey][stateName];
    if (!allDistrictData) return "";

    // Collect all date_of_data values for this district across all keys
    const dates = Object.values(allDistrictData)
      .map((row) => row?.date_of_data)
      .filter(Boolean);

    // If the current value is not "NA", return it
    const currentValue = allDistrictData[districtName]?.date_of_data;
    if (currentValue && currentValue !== "NA") return currentValue;

    // Filter out "NA" and find the latest date
    const validDates = dates.filter((date) => date && date !== "NA");
    if (validDates.length === 0) return "";

    // Find the greatest date (assuming YYYY-MM-DD format)
    return validDates.reduce((a, b) => (toISODate(a) > toISODate(b) ? a : b));
  }
  ...
  const availableKPIs = useMemo(() => {
    if (!outputdata || !selectedSchemes.length) return [];
    // For each selected scheme, collect all KPI keys (Object.keys) from outputdata[scheme]
    return Array.from(
      new Set(
  ...
  // FILTERING LOGIC: single useEffect, same structure as StateSummaryReport
  useEffect(() => {
    if (
      selectedSectors.length === 0 ||
      selectedDepartments.length === 0 ||
    ...
    // Filter by States/Districts for table rendering
